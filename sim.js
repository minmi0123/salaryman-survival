...
// 게임 시작 (시작화면 -> 새로하기 -> 인트로가 있으면 건너뛴다)
const 새게임 = H => {
  H.클릭({ title: 'new' });
  // 인트로(프롤로그)는 밸런스와 무관하다. 마지막 [시작] 을 바로 눌러 통과한다.
  if (H.g.screen === 'intro') H.클릭({ intro: 'start' });
};
